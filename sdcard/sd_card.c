...
static const uint64_t tokenTimeout = SD_COMMAND_TIMEOUT * 1000000; /* 2 seconds */


// SPI function to wait till chip is ready and sends start token
static inline bool sd_wait_token(sd_card_t *pSD, uint8_t token) {
    //TRACE_PRINTF("%s(0x%02hhx)\r\n", __FUNCTION__, token);

    u_int8_t receive = SPI_FILL_CHAR;
    uint64_t timeout = time_us_64 () + tokenTimeout;

    while ( time_us_64 () < timeout )
    {
        my_spi_read_blocking ( pSD->spi->hw_inst, &receive, 1 );
...
static int sd_read_block(sd_card_t *pSD, uint8_t *buffer, uint32_t length) {
    ...
    // read data
    if ( ! my_spi_read_blocking ( pSD->spi->hw_inst, buffer, length ) ) {
        return SD_BLOCK_DEVICE_ERROR_NO_RESPONSE;
    }
    // Read the CRC16 checksum for the data block
    crc = (sd_spi_write(pSD, SPI_FILL_CHAR) << 8);
...
static int in_sd_read_blocks(sd_card_t *pSD, uint8_t *buffer,
    ...
    if (ulSectorNumber + ulSectorCount > pSD->sectors)
    ...
    status = sd_cmd(pSD, CMD18_READ_MULTIPLE_BLOCK, ulSectorNumber, false, 0);
    ...
    /* we can optimise here - use dma for multi-sector reads */
    // receive the data : one block at a time
    int rd_status = 0;

    for ( int b = 0; b < ulSectorCount; b++ )
    {
...
static uint8_t sd_write_block(sd_card_t *pSD, const uint8_t *buffer,
    ...
    // write the data
    bool ret = my_spi_write_blocking ( pSD->spi->hw_inst, buffer, length );
...
{
    uint32_t stat = 0;
    uint32_t sectors = pSD->sectors;


    sd_spi_select ( pSD );

    /* perform checks for SDSC and SDHC card types */
    if ( pSD->card_type == SDCARD_V1 || pSD->card_type == SDCARD_V2 ) 
    {
        sectors *= _block_size;
    }

    /* set beginning and end sectors */
    sd_cmd ( pSD, CMD32_ERASE_WR_BLK_START_ADDR, 0, false, &stat );
    sd_cmd ( pSD, CMD33_ERASE_WR_BLK_END_ADDR, sectors, false, &stat );
    sd_cmd ( pSD, CMD38_ERASE, 0, false, &stat );
