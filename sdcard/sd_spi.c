...
void sd_spi_go_high_frequency(sd_card_t *pSD) {
    uint actual = spi_set_baudrate_fast(pSD->spi->hw_inst, pSD->spi->baud_rate);
...
inline uint8_t sd_spi_write(sd_card_t *pSD, const uint8_t value) {
    // TRACE_PRINTF("%s\n", __FUNCTION__);
    u_int8_t received = SPI_FILL_CHAR;

    my_spi_write_read_blocking_one ( pSD->spi->hw_inst, &value, &received, 1 );
...
void sd_spi_send_initializing_sequence(sd_card_t * pSD) {
    ...
    do {
        my_spi_write_blocking ( pSD->spi->hw_inst, ones, sizeof ones );
